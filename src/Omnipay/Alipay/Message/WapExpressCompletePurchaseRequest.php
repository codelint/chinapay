...
class WapExpressCompletePurchaseRequest extends AbstractRequest {
    ...
    {
        $this->validate('request_params', 'partner', 'key');
        $this->validateRequestParams('notify_data');
    ...
    {
        $data = array();
        $notify_data = $this->getNotifyData();
        $doc = new DOMDocument();
        ...
        if (!empty($doc->getElementsByTagName("notify")->item(0)->nodeValue))
        {
            ...
            {
                $this->verifyResponse = $this->getVerifyResponse($notify_id);
                $validateSign = true;
            }
